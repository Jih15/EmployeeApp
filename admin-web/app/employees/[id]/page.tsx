    ...
    <div style={{ width: 260, flexShrink: 0, display: "flex", flexDirection: "column", gap: 12 }}>
      ...
      <div className="card" style={{ borderRadius: "var(--r2)" }}>
        <div className="card-inner" style={{ alignItems: "center", padding: "22px 20px" }}>
          <div style={{
            ...
            background: emp.avatarBg, color: emp.avatarFc,
            display: "flex", alignItems: "center", justifyContent: "center",
            fontFamily: '"Fira Code", monospace', fontSize: 22, fontWeight: 700,
            marginBottom: 12, boxShadow: "0 4px 14px rgba(15,23,42,0.1)",
          }}>
            {emp.initials}
          </div>
          <div style={{ textAlign: "center" }}>
            <div style={{ fontFamily: '"Playfair Display", serif', fontSize: 16, fontWeight: 700, color: "var(--navy)", marginBottom: 3 }}>
              {emp.name}
            </div>
            <div style={{ fontSize: 12, color: "var(--slate2)", marginBottom: 10 }}>{emp.position}</div>
      ...
      <div className="card" style={{ borderRadius: "var(--r2)" }}>
        <div className="card-inner" style={{ padding: "14px 16px", gap: 0 }}>
          ...
          {[
            ...
            <div key={item.label} style={{ paddingBottom: 9, marginBottom: 9, borderBottom: "1px solid var(--line2)" }}>
              <div style={{ fontSize: 9.5, fontWeight: 700, letterSpacing: "0.07em", textTransform: "uppercase", color: "var(--slate2)", marginBottom: 2 }}>
                {item.label}
              </div>
              <div style={{
          ...
          <div>
            <div style={{ fontSize: 9.5, fontWeight: 700, letterSpacing: "0.07em", textTransform: "uppercase", color: "var(--slate2)", marginBottom: 6 }}>
              Kelengkapan Dokumen
            </div>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 5 }}>
              <span style={{ fontSize: 11, color: "var(--ink3)" }}>{verifiedDocs}/{emp.documents.length} dokumen</span>
              <span style={{ fontFamily: '"Fira Code", monospace', fontSize: 11, fontWeight: 600, color: docPct >= 80 ? "var(--emerald)" : "var(--amber2)" }}>{docPct}%</span>
            </div>
            <div className="prog-track">
              <div className="prog-fill" style={{ width: `${docPct}%`, background: docPct >= 80 ? "var(--emerald2)" : "var(--amber2)" }} />
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="card" style={{ borderRadius: "var(--r2)" }}>
        <div className="card-inner" style={{ padding: "14px 16px" }}>
          ...
          <div style={{ fontSize: 13, fontWeight: 600, color: "var(--navy)", marginBottom: 2 }}>{emp.emergencyContact.name}</div>
          <div style={{ fontSize: 11, color: "var(--slate2)", marginBottom: 6 }}>{emp.emergencyContact.relation}</div>
          <div style={{ fontFamily: '"Fira Code", monospace', fontSize: 11.5, color: "var(--ink3)" }}>{emp.emergencyContact.phone}</div>
        </div>
      </div>
      ...
      <div className="card" style={{ borderRadius: "var(--r2)" }}>
        <div className="card-inner" style={{ padding: "14px 16px", gap: 7 }}>
          <Link href={`/employees/${emp.id}/edit`} className="btn btn-primary" style={{ width: "100%", textDecoration: "none", justifyContent: "center" }}>
...
function TabProfil({ emp }: { emp: Employee }) {
  ...
  return (
    <div>
      ...
      {/* Kontak darurat */}
      <div style={{ background: "var(--ruby3)", borderRadius: "var(--r)", padding: "14px 16px", border: "1px solid rgba(185,28,28,0.15)" }}>
        <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: "0.07em", textTransform: "uppercase", color: "var(--ruby)", marginBottom: 10 }}>
          Kontak Darurat
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12 }}>
          {[
            ...
            <div key={f.label}>
              ...
              <div style={{ fontSize: 12.5, color: "var(--ink2)", fontFamily: f.mono ? '"Fira Code", monospace' : undefined }}>{f.val}</div>
            </div>
          ))}
        </div>
      </div>
...
function TabPekerjaan({ emp }: { emp: Employee }) {
  const fields = [
    ...
    { label: "Lama Bekerja", val: (() => {
      const diff = Date.now() - new Date(emp.joinDate).getTime();
  ...
  return (
    <div>
        ...
        {fields.map((f) => (
          <div key={f.label}>
            ...
            <div style={{
              ...
              border: "1px solid",
              borderColor: f.label === "Gaji Pokok" ? "rgba(13,124,82,0.2)" : "var(--line)",
              borderRadius: 8,
              fontSize: 12.5,
      ...
      {/* Absensi bulan ini */}
      <div style={{ background: "var(--surface2)", borderRadius: "var(--r)", padding: "14px 16px", border: "1px solid var(--line)" }}>
        <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: "0.07em", textTransform: "uppercase", color: "var(--slate)", marginBottom: 12 }}>
          Ringkasan Absensi · {emp.attendanceSummary.month}
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(5,1fr)", gap: 10 }}>
          {[
            { label: "Hadir", val: emp.attendanceSummary.hadir, color: "var(--emerald)", bg: "var(--emerald3)" },
            { label: "Telat", val: emp.attendanceSummary.telat, color: "var(--amber)", bg: "var(--amber3)" },
            { label: "Izin", val: emp.attendanceSummary.izin, color: "var(--sapphire)", bg: "var(--sapphire4)" },
            { label: "Cuti", val: emp.attendanceSummary.cuti, color: "#7c3aed", bg: "#ede9fe" },
            { label: "Alpha", val: emp.attendanceSummary.alpha, color: "var(--ruby)", bg: "var(--ruby3)" },
          ].map((item) => (
            <div key={item.label} style={{ background: item.bg, borderRadius: 8, padding: "10px 12px", textAlign: "center" }}>
              <div style={{ fontFamily: '"Playfair Display", serif', fontSize: 22, fontWeight: 700, color: item.color }}>{item.val}</div>
              <div style={{ fontSize: 11, color: item.color, marginTop: 2 }}>{item.label}</div>
            </div>
          ))}
        </div>
        <div style={{ marginTop: 10, fontFamily: '"Fira Code", monospace', fontSize: 10, color: "var(--slate2)" }}>
          Total hari kerja: {emp.attendanceSummary.totalHari} hari ·{" "}
          Tingkat kehadiran: {Math.round((emp.attendanceSummary.hadir / emp.attendanceSummary.totalHari) * 100)}%
        </div>
      </div>
    </div>
  );
    ...
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
        <div style={{ fontSize: 12, color: "var(--slate2)" }}>
          {emp.documents.filter((d) => d.status === "verified").length} dari {emp.documents.length} dokumen terverifikasi
        </div>
        ...
        {emp.documents.map((doc) => {
          ...
          return (
            <div key={doc.id} style={{
              ...
              {/* Info */}
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 13, fontWeight: 600, color: "var(--navy)", marginBottom: 2 }}>{doc.name}</div>
                <div style={{ fontFamily: '"Fira Code", monospace', fontSize: 10, color: "var(--slate2)" }}>
              ...
              {/* Type badge */}
              <span style={{
                fontFamily: '"Fira Code", monospace', fontSize: 9.5, padding: "2px 8px",
                background: "var(--surface3)", borderRadius: 4, color: "var(--slate)",
              }}>
                {doc.type}
              </span>

              {/* Status */}
              <span className={`badge ${ds.cls}`}><span className="bdot" />{ds.label}</span>
    ...
    <div>
      <div style={{ position: "relative", paddingLeft: 28 }}>
        ...
        {emp.careerHistory.map((h, i) => {
          ...
          return (
            <div key={h.id} style={{ position: "relative", marginBottom: i < emp.careerHistory.length - 1 ? 20 : 0 }}>
              {/* Dot */}
              <div style={{
                position: "absolute", left: -24, top: 10,
                width: 16, height: 16, borderRadius: "50%",
                background: hc.bg, border: `2px solid ${hc.color}`,
                display: "flex", alignItems: "center", justifyContent: "center",
                fontSize: 8, zIndex: 1,
              }}>
                <span>{hc.icon}</span>
              ...
              <div style={{
                ...
                <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 12 }}>
                  <div>
                    <span className="badge" style={{ background: hc.bg, color: hc.color, marginBottom: 6 }}>
                      {{ join: "Bergabung", promotion: "Promosi", salary: "Penyesuaian Gaji", transfer: "Mutasi", status: "Perubahan Status" }[h.type]}
                    </span>
                    <div style={{ fontSize: 13, color: "var(--ink2)", lineHeight: 1.4 }}>{h.description}</div>
                    <div style={{ fontFamily: '"Fira Code", monospace', fontSize: 10, color: "var(--slate2)", marginTop: 6 }}>
                      Oleh: {h.by}
                    </div>
                  </div>
                  <span style={{ fontFamily: '"Fira Code", monospace', fontSize: 10.5, color: "var(--slate2)", flexShrink: 0 }}>
                    {h.date}
                  </span>
                </div>
              </div>
...
export default function EmployeeDetailPage({ employee }: EmployeeDetailPageProps) {
  ...
  return (
    <div id="content">
      {/* Breadcrumb */}
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 14, fontFamily: '"Fira Code", monospace', fontSize: 11, color: "var(--slate2)" }}>
        <Link href="/employees" style={{ color: "var(--sapphire)", textDecoration: "none" }}>Karyawan</Link>
        <span>›</span>
        <span style={{ color: "var(--ink3)" }}>{employee.name}</span>
        <span style={{ fontFamily: '"Fira Code", monospace', fontSize: 10, background: "var(--surface3)", padding: "1px 6px", borderRadius: 4 }}>
          {employee.id}
        </span>
      </div>
      ...
      <div style={{ display: "flex", gap: 16, alignItems: "flex-start" }}>
        ...
        <div style={{ flex: 1, minWidth: 0, display: "flex", flexDirection: "column", gap: 0 }}>
          <div className="card" style={{ borderRadius: "var(--r3)", animation: "cardIn 0.4s ease forwards", opacity: 0 }}>
                ...
                {tabs.map((tab) => (
                  <button key={tab.key} onClick={() => setActiveTab(tab.key)}
                    style={{
                      ...
                      color: activeTab === tab.key ? "var(--sapphire)" : "var(--slate)",
                      borderBottom: activeTab === tab.key ? "2px solid var(--sapphire)" : "2px solid transparent",
                      marginBottom: "-1px",
                      transition: "all 0.15s",
                    }}>
                    {tab.label}
